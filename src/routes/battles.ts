...
router.get('/random-challenge', [
  ...
  try {
    ...
    const trainer1 = await DatabaseService.getTrainer(parseInt(String(req.query.trainer1Id)));
    const trainer2 = await DatabaseService.getTrainer(parseInt(String(req.query.trainer2Id)));
    ...
    const interval = setInterval(async () => {
      if (!pokemon1.isAlive() || !pokemon2.isAlive() || rounds >= maxRounds) {
        ...
        await DatabaseService.updateTrainer(winner);
        await DatabaseService.updateTrainer(loser);
    ...
    req.on('close', () => clearInterval(interval as any));
  } catch (error) {
